...
export class GMSAnalysisEngine {
  ...
  suggestTradeOffers(myTeam,targetTeam,targetPlayer,targetPick,context={}){
    ...
    const targetPlayerValue=playerValue(targetPlayer),targetValue=targetPlayerValue==null?null:targetPlayerValue+(pickValue??0),requestedPackage=targetPick?`${targetPlayer.name} plus ${pickYear??"unknown year"} Round ${pickRound??"unknown"} pick`:`${targetPlayer.name}`;
    ...
    return(myTeam.players||[]).filter(p=>!this._ir(p)&&this._score(p)!=null).map(player=>{
      ...
      const reasons=[
        gap==null?`Value gap is unavailable because the target player, pick, or offer asset lacks scoring/round data.`:`${player.name} carries an evidence value ${gap>=0?"+":""}${gap.toFixed(1)} from the requested ${requestedPackage}.`,
