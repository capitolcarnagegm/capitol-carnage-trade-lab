// Pride boot — Fantrax direct on gmslocker.com
(function () {
  ...
  function pickLabel(year, round, originalName) {
    ...
    return originalName ? base + " (" + originalName + ")" : base;
  }
  ...
  async function loadLeague() {
    ...
    try {
      ...
      var rosterMap = rostersPayload.rosters || {};
      ...
      db.fantraxLiveMeta = {
        ...
        teamCount: franchises.length, pickCount: picksAdded, source: "Fantrax direct"
      };
      ...
      if (health) {
        health.innerHTML = "<b>GM's Locker v8.1</b> · <span class=\"good\">" +
          franchises.length + " teams · " + picksAdded + " picks · Fantrax live</span>";
      }
      console.log("[gmslocker] loaded", franchises.length, "teams", created, "new", picksAdded, "picks");
      if (typeof window.renderTeamsPage === "function") {
        try { window.renderTeamsPage(); } catch (e) {}
      }
    } catch (e) {
      console.warn("[gmslocker] load failed", e);
      var health = document.getElementById("buildHealth");
      if (health) health.innerHTML = "<b>GM's Locker v8.1</b> · <span class=\"bad\">" + String(e.message || e) + "</span>";
      if (tries < 5) setTimeout(loadLeague, 2500);
    }
  }
