(function () {
  ...
  function diagnosticsHtml() {
    ...
    return '<div class="card live-data-health"><div class="sectionhead"><h2>Live Data Health</h2><span class="pill">' + (warnings.length ? 'PARTIAL' : 'SYNCED') + '</span></div>' +
      ...
      (warnings.length ? '<div class="error-banner"><b>Partial Fantrax sync:</b> ' + escapeHtml(warnings.join(' · ')) + '</div>' : '<div class="notice"><b>Verified:</b> roster assignments and the waiver pool above came from the current Fantrax response. No sample players are counted here.</div>') +
      '</div>';
  }
  ...
  function scheduleHtml() {
    ...
    var upcoming = games.filter(function (game) { return game.state === "pre"; });
    var display = (upcoming.length ? upcoming : games).slice(0, 20);
    var html = '<div class="card nfl-schedule"><div class="sectionhead"><div><h2>NFL Schedule</h2><span class="small muted">Upcoming games load even before box scores exist</span></div><span class="pill">' + games.length + ' GAMES</span></div>';
  ...
  function refreshDiagnostics() {
    ...
    var view = current && current.getAttribute('data-view');

    var oldHealth = main.querySelector('.live-data-health');
    if (oldHealth) oldHealth.remove();
    if (latestLeagueData && (view === 'leagues' || view === 'team' || view === 'waivers')) {
  ...
  document.addEventListener('click', function () { setTimeout(refreshDiagnostics, 60); });
  new MutationObserver(function () { setTimeout(refreshDiagnostics, 0); }).observe(document.documentElement, { childList: true, subtree: true });
})();
