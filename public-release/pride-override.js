...
(function () {
  ...
  const GATEWAY = "https://gms-locker-ai.robinharvey001.workers.dev";
  let liveCache = null;
  ...
  async function forceLoadFantraxLeague() {
      ...
      (team.rosterItems || []).forEach(function (item) {
        ...
        if (!asset) {
          asset = {
            ...
            market: Math.max(200, salary * 40), adp: 0, ecr: 0, proj: Number(item.proj || 0), ppg: 0, role: 55, trend: 0, risk: 40, liquidity: 55,
            salary: salary, years: years, roster: teamName, status: status, source: "Fantrax Live",
            weeklyProj: Number(item.weeklyProj || 0),
          ...
          if (item.proj) asset.proj = Number(item.proj);
          if (item.weeklyProj) asset.weeklyProj = Number(item.weeklyProj);
    ...
    if (health) {
      health.innerHTML = "<b>GM's Locker</b> · <span class=\"good\">" + franchises.length + " teams · " + rosteredIds.size + " rostered from Fantrax</span>";
    }
    return payload;
  }
  ...
  function allTeams() {
    const names = new Set();
    const f = (typeof db !== "undefined" && db.mfl && db.mfl.franchises) || [];
    f.forEach(function (x) { if (x && x.name) names.add(x.name); });
    if (typeof db !== "undefined" && db.assets) {
      db.assets.forEach(function (a) {
        if (a.roster && !/free agent|^fa$/i.test(String(a.roster))) names.add(a.roster);
      });
    }
    if (!names.size && liveCache && liveCache.snapshots && liveCache.snapshots.rosters) {
      var rm = liveCache.snapshots.rosters.rosters || {};
      Object.keys(rm).forEach(function (id) {
        if (rm[id] && rm[id].teamName) names.add(rm[id].teamName);
      });
    }
