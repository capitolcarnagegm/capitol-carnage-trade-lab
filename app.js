...
(function () {
  ...
  var state = {
    account: null, access: null, ownerAccounts: [], workspaces: [], activeWorkspace: null, authMode: "login", authUsername: "", authEmail: "", authError: "", onboarding: null,
  ...
  window.GMS = {
    ...
    setAuthMode: function (mode) { state.authMode = mode === "register" ? "register" : "login"; state.authError = ""; render(); },
    register: async function () { var username = document.getElementById("authUsername"), email = document.getElementById("authEmail"), password = document.getElementById("authPassword"); state.authUsername = String(username && username.value || "").trim(); state.authEmail = String(email && email.value || "").trim(); state.authError = ""; try { var data = await postJson("/auth/register", { username: state.authUsername, email: state.authEmail, password: password && password.value }); AUTH_TOKEN = data.token; SESSION_STORE.setItem("gms_session", AUTH_TOKEN); state.account = data.user; await loadAccount(); } catch (error) { state.authError = String(error.message || error); render(); } },
