...
(function () {
  ...
  function starterSlots() {
    ...
    BYLAWS.starters.forEach(function (spec) {
      for (var n = 0; n < spec.count; n++) slots.push({ slot: spec.count > 1 ? spec.slot + " " + (n + 1) : spec.slot, accept: spec.accept.slice() });
    });
    return slots;
  }
  ...
  function capProjection(teamName, cutIds) {
    ...
    var fiveYearCutProjection = [0, 1, 2, 3, 4].map(function (offset) {
      ...
      return { offset: offset, year: currentYear + offset, leagueCap: cap * Math.pow(1 + BYLAWS.capInflation, offset),
  ...
  function authScreen() { var registering = state.authMode === "register"; return '<div class="account-shell"><div class="card account-card"><span class="pill">PRIVATE ACCOUNT</span><h1>' + (registering ? 'Create your GMS Locker account' : 'Sign in to GMS Locker') + '</h1><p>Your leagues, teams, settings, and chats stay separated from every other GM.</p>' + (state.authError ? '<div class="error-banner">' + esc(state.authError) + '</div>' : '') + '<div class="field"><label>Username</label><input id="authUsername" autocomplete="username" maxlength="30" placeholder="your_username" value="' + esc(state.authUsername) + '"></div>' + (registering ? '<div class="field"><label>Email address</label><input id="authEmail" type="email" autocomplete="email" placeholder="you@example.com" value="' + esc(state.authEmail) + '"></div>' : '') + (registering && state.authLegacy ? '<div class="field"><label>Six-digit email code</label><input id="authCode" inputmode="numeric" autocomplete="one-time-code" maxlength="6" placeholder="000000"></div>' : '') + '<div class="field"><label>Password</label><input id="authPassword" type="password" autocomplete="' + (registering ? 'new-password' : 'current-password') + '" maxlength="128" placeholder="At least 10 characters"></div><button class="primary wide" onclick="GMS.' + (registering ? 'register' : 'login') + '()">' + (registering ? (state.authLegacy ? 'Verify and keep my account' : 'Create account') : 'Sign in') + '</button><button class="secondary wide" onclick="GMS.setAuthMode(\'' + (registering ? 'login' : 'register') + '\')">' + (registering ? 'Already have an account? Sign in' : 'New here? Create an account') + '</button><p class="small muted">Passwords are salted and hashed. GMS Locker never asks for your Fantrax password.</p></div></div>'; }
  function paymentScreen() { return '<div class="account-shell"><div class="card account-card"><span class="pill">12-MONTH ACCESS</span><h1>Unlock GMS Locker</h1><p><b>$49.99 once for your entire account.</b> League data and analysis remain locked until payment is confirmed.</p>' + ((state.access && state.access.settings.checkout === false) ? '<div class="notice">Checkout is temporarily unavailable. Contact gmslocker@gmail.com.</div>' : mvpCheckout('paywall')) + '<button class="secondary wide" onclick="GMS.logout()">Sign out</button></div></div>'; }
  ...
  async function postJson(path, body) { var response = await fetch(API_BASE + path, { method: "POST", headers: authHeaders({ "Content-Type": "application/json" }), body: JSON.stringify(body || {}) }); var data = await response.json(); if (!response.ok) { var error = new Error(data.detail || data.error || "Request failed"); error.code = data.code || ""; throw error; } return data; }

  window.GMS = {
    loginOrLogout: function () { if (state.account) GMS.logout(); else { state.authError = ""; render(); } },
    toggleControl: async function (key, enabled) { try { var data = await postJson("/owner/controls", { key: key, enabled: enabled }); state.access.settings = data.settings; render(); } catch (error) { alert(error.message || error); } },
    entitle: async function (action) { var input = document.getElementById("ownerEmail"), email = String(input && input.value || "").trim(); if (!email) return; try { await postJson("/owner/entitlements", { email: email, action: action }); alert("Access updated for " + email); } catch (error) { alert(error.message || error); } },
    ...
    setAuthMode: function (mode) { state.authMode = mode === "register" ? "register" : "login"; state.authLegacy = false; state.authError = ""; render(); },
    register: async function () { var username = document.getElementById("authUsername"), email = document.getElementById("authEmail"), password = document.getElementById("authPassword"), code = document.getElementById("authCode"); state.authUsername = String(username && username.value || "").trim(); state.authEmail = String(email && email.value || "").trim(); state.authError = ""; try { var data = await postJson("/auth/register", { username: state.authUsername, email: state.authEmail, password: password && password.value, code: code && code.value }); AUTH_TOKEN = data.token; SESSION_STORE.setItem("gms_session", AUTH_TOKEN); state.account = data.user; state.authLegacy = false; await loadAccount(); } catch (error) { if (error.code === "LEGACY_VERIFICATION_REQUIRED") state.authLegacy = true; state.authError = String(error.message || error); render(); } },
    login: async function () { var username = document.getElementById("authUsername"), password = document.getElementById("authPassword"); state.authUsername = String(username && username.value || "").trim(); state.authError = ""; try { var data = await postJson("/auth/login", { username: state.authUsername, password: password && password.value }); AUTH_TOKEN = data.token; SESSION_STORE.setItem("gms_session", AUTH_TOKEN); state.account = data.user; await loadAccount(); } catch (error) { state.authError = String(error.message || error); render(); } },
